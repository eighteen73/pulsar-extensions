<?php
/**
 * Term Template Grid extension class.
 *
 * @package Eighteen73\PulsarExtensions
 */

namespace Eighteen73\PulsarExtensions\Extensions\TermTemplate;
...
class Grid {
	...
	public function setup(): void {
		add_filter( 'render_block_core/term-template', [ $this, 'render_grid_classes' ], 10, 2 );
	}
	...
	private function apply_classes_and_styles( string $block_content, string $classes, string $styles ): string {
		...
		// Find the <ul> tag with wp-block-term-template class
		if ( ! $processor->next_tag( [ 'class_name' => 'wp-block-term-template' ] ) ) {
			return $block_content;
		}
